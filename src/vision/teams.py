...
class TeamClassifier:
    ...
    def classify(self, clip_path: str, track: TrackResult) -> dict[int, int]:
        ...
        # collect one feature per (frame, player) then aggregate per track id
        ...
        while True:
            ...
            if fd:
                for p in fd.players:
                    f = self._features(frame, p["xyxy"], cv2)
                    if f is not None:
                        feats.append(f)
                        owners.append(p["id"])
            idx += 1
        ...
        log.info(f"[teams] assigned {len(team_of)} tracks to {self.n_teams} teams")
        return team_of
