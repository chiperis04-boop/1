...
## Дополнение — AI-driven viral tuning (референс Lamine Yamal)

Проход по продуктовому видению: AI-агенты (Scout/Director/Critic) принимают
контекстные/творческие/тайминговые решения, а стабилизированные пресеты остаются
жёсткими рельсами рендера. 26-параметрный инвариант WebUI (коммит 2cdd026) НЕ
тронут (проверено AST: render_job 26==26, preview 10==10).

Тюнинг под референс-эстетику (config/config.yaml):
- Кроп на легах+мяче ~68% высоты (`edit.reframe.target_subject_height: 0.45→0.68`).
- Камера ведёт мяч на 15% вектора скорости (`lead_gain: 0.25→0.15`), лимиты пана/
  зума и per-shot сброс Кальмана — уже были (Блок C).
- Неон-шлейф: FLUSH при скачке >5% ширины (`trail_max_jump_frac: 0.18→0.05`) —
  «линии в небо» исключены; тонкий (2px) тейпер + grounded нимб под ботсами — были.
- Субтитры ≤3% высоты (`captions.hook_scale: 0.032→0.030`), 1px drop-shadow,
  30% скруглённая подложка, ≤2 строки, и хук ПОД табло (`avoid_scoreboard: true`,
  safe-zone верх 18%). Подтверждено ПО КАДРАМ (scripts/render_check.py).

Новая AI-логика (расширены реальные модули, с тестами):
- Scout (`event_feed.py`): скан ключевых слов скилла (dribble/nutmeg/solo run/
  pace/…) + energy-скоринг описания (`_skill_energy`) ДИНАМИЧЕСКИ расширяет окно
  клипа (`energy_pre_boost`/`energy_post_boost`) — «outrageous solo run 🔥» даёт
  более длинное окно, чем рутинный dribble. Тест `test_skill_energy_dynamic_window`.
- Music (`music.py`): per-beat accent (onset-strength) в `BeatGrid.strengths` +
  `strong_beats()` (бас-дропы = топ-fraction по акценту); `beat_align_plan`
  сажает AI-слоумо на ближайший бас-дроп (шире толеранс `beat_drop_max_shift`).
  Тесты `test_strong_beats_selects_accented`, `test_beat_align_lands_on_bass_drop`.
- Director (`director.py`): энергичный офлайн-хук для скилла, использующий имя
  игрока из лога («YAMAL IN HACKER MODE») — имя из данных, не выдумано; богатый
  креативный хук пишет VLM (verify on GPU). Тест `test_default_hook_skill_is_energetic`.
- Critic/review (`review.py`): issue `text_in_ui`/`text_unreadable` теперь ТРИГГЕРИТ
  single-pass ревизию — `caption_safe=True` + `hook_scale_mult*=0.9`, прокинуто в
  рендер (`studio_pipeline.render_plan` пересобирает hook_scale/avoid_scoreboard из
  базовых значений без накопления). Тест `test_apply_corrections_text_overlap`.

Границы честности: агрегаты YOLO detect/track/seg и Qwen2.5-VL Director/Critic —
по-прежнему **verify on GPU** (в песочнице нет GPU/Modal); синтетический
render-check доказывает ПОВЕДЕНИЕ оверлеев/кропа/кодека/тайминга, не эстетику
живого матча. Все 16 тест-сьютов зелёные, compileall чистый.
